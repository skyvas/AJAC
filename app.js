...
(function () {
  ...
  function setStoredPassword(slug, password) {
    try {
      ...
      console.warn('LocalStorage password write error:', e);
    }
  }
  ...
  async function loadData() {
    ...
    applications = rawApps.map(app => {
      ...
      if (cached) {
        ...
        const password = (cached.password !== undefined) ? cached.password : (app.password || '');
        return {
          ...app,
          status,
  ...
  async function updateApplicationPassword(slug, newPassword) {
    const app = applications.find(a => a.slug === slug);
    if (!app) return;

    const trimmed = (newPassword || '').trim();
    if (trimmed === (app.password || '')) return;

    app.password = trimmed;
    app.updated_at = new Date().toISOString();
    ...
    if (card) {
      const indicator = card.querySelector('.pwd-saved-indicator');
      if (indicator) {
        indicator.textContent = trimmed ? 'Saved!' : 'Cleared';
  ...
  function render() {
    ...
    if (searchQuery) {
      ...
      targetApps = targetApps.filter(app => {
        ...
        const matchPassword = (app.password || '').toLowerCase().includes(q);
        return matchCompany || matchTitle || matchLocation || matchDate || matchSlug || matchPassword;
      });
    }
  ...
  function renderCardHtml(app) {
    ...
    const pwdVal = app.password || '';
    const hasPassword = pwdVal.length > 0;
      ...
      <article class="app-card ${stateClass}" id="card-${app.slug}" data-slug="${app.slug}">
        <div class="card-content">
          ...
          <div class="card-info">
            ...
            <div class="card-password-row" data-slug="${app.slug}">
              <label class="password-label" for="pwd-${app.slug}">
                <svg class="password-icon" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect>
                  <path d="M7 11V7a5 5 0 0 1 10 0v4"></path>
                </svg>
                <span>Password:</span>
              </label>
              <div class="password-input-group ${hasPassword ? 'has-value' : ''}">
                <input 
                  type="text" 
                  id="pwd-${app.slug}"
                  class="password-input" 
                  placeholder="Add portal password (optional)" 
                  value="${escapeHtml(pwdVal)}" 
                  data-original="${escapeHtml(pwdVal)}"
                  data-slug="${app.slug}"
                  autocomplete="off" 
                  spellcheck="false"
                  aria-label="Portal password for ${escapeHtml(app.company)}"
                />
                <button type="button" class="pwd-action-btn btn-save-pwd hidden" data-action="save-pwd" title="Save password">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                    <polyline points="20 6 9 17 4 12"></polyline>
                  </svg>
                  <span>Save</span>
                </button>
                <button type="button" class="pwd-action-btn btn-copy-pwd ${!hasPassword ? 'hidden' : ''}" data-action="copy-pwd" title="Copy password to clipboard">
                  <svg class="icon-copy" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect>
                    <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path>
                  </svg>
                  <span class="copy-text">Copy</span>
                </button>
                <button type="button" class="pwd-action-btn btn-toggle-mask ${!hasPassword ? 'hidden' : ''}" data-action="toggle-mask" title="Toggle password mask">
                  <svg class="icon-eye-open" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>
                    <circle cx="12" cy="10" r="3"></circle>
                  </svg>
                  <svg class="icon-eye-closed hidden" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path>
                    <line x1="1" y1="1" x2="23" y2="23"></line>
                  </svg>
                </button>
                <button type="button" class="pwd-action-btn btn-clear-pwd ${!hasPassword ? 'hidden' : ''}" data-action="clear-pwd" title="Clear password">
                  &times;
                </button>
              </div>
              <span class="pwd-saved-indicator hidden">Saved!</span>
            </div>
          </div>
  ...
  function attachCardListeners(scope = document) {
    ...
    // Password Inputs & Action Listeners
    const pwdInputs = scope.querySelectorAll('.password-input');
    pwdInputs.forEach(input => {
      const row = input.closest('.card-password-row');
      const slug = input.getAttribute('data-slug');
      const saveBtn = row ? row.querySelector('.btn-save-pwd') : null;

      input.addEventListener('input', () => {
        const currentVal = input.value.trim();
        const origVal = (input.getAttribute('data-original') || '').trim();
        if (saveBtn) {
      ...
      input.addEventListener('keydown', e => {
        if (e.key === 'Enter') {
          e.preventDefault();
          updateApplicationPassword(slug, input.value);
      ...
      input.addEventListener('blur', e => {
        // If clicking inside the same password row, don't trigger auto-save blur immediately
        if (e.relatedTarget && row && row.contains(e.relatedTarget)) {
          return;
        }
        const currentVal = input.value.trim();
        const origVal = (input.getAttribute('data-original') || '').trim();
        if (currentVal !== origVal) {
          updateApplicationPassword(slug, currentVal);
        }
      });
    });

    const savePwdBtns = scope.querySelectorAll('[data-action="save-pwd"]');
    savePwdBtns.forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        const row = btn.closest('.card-password-row');
        const input = row ? row.querySelector('.password-input') : null;
        if (input) {
          const slug = input.getAttribute('data-slug');
          updateApplicationPassword(slug, input.value);
        }
    ...
    const copyPwdBtns = scope.querySelectorAll('[data-action="copy-pwd"]');
    copyPwdBtns.forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        const row = btn.closest('.card-password-row');
        const input = row ? row.querySelector('.password-input') : null;
    ...
    const toggleMaskBtns = scope.querySelectorAll('[data-action="toggle-mask"]');
    toggleMaskBtns.forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        const row = btn.closest('.card-password-row');
        const input = row ? row.querySelector('.password-input') : null;
        ...
        if (input.type === 'text') {
          ...
          input.type = 'text';
          btn.setAttribute('title', 'Mask password');
    ...
    const clearPwdBtns = scope.querySelectorAll('[data-action="clear-pwd"]');
    clearPwdBtns.forEach(btn => {
      btn.addEventListener('click', e => {
        e.stopPropagation();
        const row = btn.closest('.card-password-row');
        const input = row ? row.querySelector('.password-input') : null;
        if (input) {
          const slug = input.getAttribute('data-slug');
          updateApplicationPassword(slug, '');
        }
      });
    });
  }
